...
# Experimental details
 Virus:     Progeny  DCV grown in the absence and presence o r Wolbachia (W- or W+)
...
# Plot
LMLplot <- plot(survfit(Surv(Days, Survival) ~ WolVirus, data = data_DCV_progeny_noPBS), fun = "cloglog")
...
* The HR of flies with W+ progeny virus is 0.8968857 (coxme) and as the CI crosses 1 there is no difference 
* For each time point there is no difference in the probability of death between flies infected with W- and W+ progeny virus 
...
```{r Plotting Kaplan-Meire curves}
...
write.csv(df_cox, file = "Survival_Progeny_DCV_Figure_4_B_coxme_HR_output.csv", row.names = FALSE)

# Saving coxme fit output 
sink("Survival_Progeny_DCV_Figure_4_B_coxme_summary_output.txt") # change file name
summary(fit1) # prints coxme output to txt file UP TO HERE
sink() # closes the connection **CRITICAL**


# Saving coxme plot 
ggsave("Survival_Progeny_DCV_Figure_4_B_coxme_plot_output.pdf", 
       plot = plot_coxme, 
       height = 7.5, 
       width = 15) 

# Saving Kaplan-Meire survival curves 
ggsave("Survival_Progeny_DCV_Figure_4_B_curves_plot_output.pdf", 
