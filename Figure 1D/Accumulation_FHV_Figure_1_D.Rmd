...
data_MNE_sub <- subset(data_MNE, Time == "3") 
```

# Graphing 
```{r Graphing}
FHV <-  ggplot(data_MNE_sub, aes(x = Line, y = log_10_FHV_MNE)) + 
  ...
  stat_summary(fun.data = mean_se, 
               ...
               size = 1,
               width = 0.3, 
               aes(group = Line, col = Line)) +
  stat_summary(fun = mean,
               geom = "crossbar",
               width = 0.15,
               position = position_dodge(width = 0.3), 
               aes(group = Line, col = Line)) +
  labs(x = NULL, y = expression(bold("Relative levels of FHV RNA")), color = "Group") +
...
FHV accumulation is only compared at 1 time point therefore a simple t-test should be sufficient. 


```{r FHV T-test}
# Removing outliers (#sample 11)
data_MNE_sub_outlier <- subset(data_MNE_sub, Sample != 11)

# T-test
FHV_t_test <- t.test(log_10_FHV_MNE ~ Line, data = data_MNE_sub)
FHV_t_test
print(FHV_t_test)

# T-test without outlier 
FHV_t_test_out <- t.test(log_10_FHV_MNE ~ Line, data = data_MNE_sub_outlier)
FHV_t_test_out
``` 

# Results from analysis 

* Mean normalised expression of FHV is different between the two groups. 
* Wolbachia has an impact on the levels of FHV after 3 hour post infection. 
...
3. No significant outliers = FAIL (outlier in the Wolbachia data # sample 11, however no impact of removing this outlier on the overall conclusions of this analysis) 
...
ggqqplot(FHV_0$log_10_FHV_MNE) 
ggqqplot(FHV_1$log_10_FHV_MNE) 

# 3. Outliers 
# In the data from Flies withtout Wolbachia
dixonTest(FHV_0$log_10_FHV_MNE)

# In data from flies with Wolbachia 
dixonTest(FHV_1$log_10_FHV_MNE) # Sample 11 Value -1.15 
...
sink("Accumulation_FHV_Figure_1_D_stats_output.txt") # add date and data_IFB_01 name | creates the txt file in WD
print(FHV_t_test) # prints coxme output to txt file
