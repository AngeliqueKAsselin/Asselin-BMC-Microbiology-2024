---
title: "Figure 5C: Accumulation of progeny FHV in Drosophila cells without Wolbachia"
date: "2023-09-19"
...
# Experimental details
 Virus:     FHV
 Titre:     100 total FHV particles per cell
...
# Approach 
Calculate mean normalised expression of FHV RNA genome relative to Drosophila melanogaster house keeping gene RpL32. 
Conduct an ANOVA to see if mean normalised expression of FHV RNA genome is different with Time and Wolbachia
...
data_FHV_acc<- as.data.frame(read_csv("Accumulation_FHV_Figure_5_C_data.csv"))

# Importing sample ID information
data_ID <- as.data.frame(read_csv("Accumulation_FHV_Figure_5_C_data_sample_ID.csv"))
```
...
Data was quality controlled before entry so there should be no data censored unless RNA is non-detectable by the assay. 
```{r Censored data }
...
data_MNE$Time <- as.factor(data_MNE$Time) # Time as a factor
data_MNE$FHV_MNE <- as.numeric(data_MNE$FHV_MNE) # MNE as a continous numeric

# Log-transforming data
data_MNE <- data_MNE %>% 
  mutate(log_10_FHV_MNE = log10(FHV_MNE)) # log10 transformation 

# Subsetting relevant info
data_anova <- data_MNE %>% 
  select(log_10_FHV_MNE, Time, Line, Replicate, Sample) %>% 
...
# Fitting the model 
FHV_aov1 <- aov(log_10_FHV_MNE ~ 
                  Line * 
                  Time , 
                data = data_anova)

FHV_aov2 <- aov(log_10_FHV_MNE ~ 
                  Line + 
                  Time , 
                data = data_anova) # Best fit model

FHV_aov3 <- aov(log_10_FHV_MNE ~ 
                  Time , 
                data = data_anova)

FHV_aov4 <- aov(log_10_FHV_MNE ~ 
                  Line , 
                data = data_anova)


# Finding the best fit model
AIC(FHV_aov1, FHV_aov2) 
AIC(FHV_aov2, FHV_aov3)
AIC(FHV_aov2, FHV_aov4)
...
shapiro.test(x = residuals(FHV_aov2))

# 3. Outliers 
plot(FHV_aov2, 1)
outlierTest(FHV_aov2)
```
# Graphing - ANOVA

```{r Graphing with Loess}
# Converting Time back into continous for the graph
data_anova$Time <- as.character(data_anova$Time)
data_anova$Time <- as.double(data_anova$Time)

# Creating plot object
FHV_plot <-ggplot(data_anova) +
  geom_jitter(
    aes(Time, log_10_FHV_MNE, col = Line),
    size = 6,
    position = position_jitter(width = 0.01),
    show.legend = FALSE
  ) +
  geom_smooth(aes(Time, log_10_FHV_MNE, group = Line, col = Line), show.legend = FALSE, inherit.aes = FALSE, method = 'loess', span = 1) +
...
sink("Accumulation_FHV_Figure_5_C_output_stats_anova.txt") # change file name
summary(FHV_aov2) # prints  output to txt file
sink() # closes the connection **CRITICAL**

sink("Accumulation_FHV_Figure_5_C_output_stats_anova_call.txt") # change file name
print(FHV_aov2) # prints  output to txt file
sink() # closes the connection **CRITICAL**

# Exporting figure
ggsave(filename = "Accumulation_FHV_Figure_5_C_output_plot.pdf",
       plot = FHV_plot, 
       width = 15, 
       height = 7.5)
