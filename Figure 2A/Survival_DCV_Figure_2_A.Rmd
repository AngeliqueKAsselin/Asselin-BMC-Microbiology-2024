---
title: "Figure 2A: Survival DCV Original "
date: "`r Sys.Date()`"
...
```{r Importing data}
data_DCV <- read.csv("Survival_DCV_Figure_2_A_data.csv")
```
...
curves_PBS <- survfit(Surv(Days, Survival) ~ Wol+ Virus, data=data_DCV)
```
...
# without PBS control
graph_DCV_PBS <- ggsurvplot(curves_PBS,
           data = data_DCV,         
           axes.offset = FALSE,            #changes axes to start at zero
           line = c(2,1,2,1),
           xlim = c(0,25),
...
# With PBS
ggsave("Survival_DCV_Figure_2_A_output_plot_survival_curves_PBS.pdf", 
       plot = graph_DCV_PBS$plot, 
       height = 7.5, 
       width = 15)
```